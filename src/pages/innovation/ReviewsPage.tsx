...
export function ReviewsPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      const [reviewsData, projectsData, teamsData] = await Promise.all([
        getReviews(),
        getProjects(),
        getTeams(),
      ]);
  ...
  const getTeamName = (teamId: number | null) =>
    teamId ? teams.find((t) => t.id === teamId)?.name || 'Unknown' : 'Solo';
  ...
  const handleDecision = (
    ...
    toast({
      ...
      description: `Project has been ${decision.replace('_', ' ')}.`,
    });
                  ...
                  {selectedReview.status === 'pending' && (
                    <>
                      ...
                      <DialogFooter className="gap-2">
                        ...
                        <Button
                          variant="outline"
                          onClick={() => handleDecision('revisions_needed')}
                        >
                          Request Revisions
                        </Button>
                        <Button
